...
def main():
    ...
    for idx, batch in enumerate(test_loader):
        ...
        if 'videofi' in args.task:
            # output = output[:, :1, ...]只取第一帧图片
            # batch['lq_path'] = batch['gt_path'][0]
            output = output[:, :, ...]
            batch['lq_path'] = [batch['gt_path'][0]] * output.shape[1]  
        ...
        if 'videofi' in args.task: gt = None
        ...
        for i in range(output.shape[1]):
            ...
            if args.save_result:
                # seq_ = osp.basename(batch['lq_path'][i][0]).split('.')[0]#原码

                #=============================================================#
                seq_ = ""
                if 'videofi' in args.task:
                    # 手动生成连续序号，不用读取路径（VFI输出多帧时路径列表不全）
                    # 获取当前序列名（比如calendar/city）
                    current_seq = folder[0]
                    # 如果是新序列，初始化计数器为1；否则沿用当前计数
                    if 'videofi' in args.task:
                        # 获取当前序列名（calendar/city等）
                        current_seq = folder[0]
                                    # 初始化该序列的计数器（从0开始）
                        seq_ = f"{seq_counter[current_seq]:08d}"

                        seq_counter[current_seq] += 1

                else:
                    seq_ = osp.basename(batch['lq_path'][i][0]).split('.')[0]
                #==============================================================#
                # 确保保存目录存在
                save_seq_dir = f'{save_dir}/{folder[0]}'
                os.makedirs(save_seq_dir, exist_ok=True)
                    # 拼接完整保存路径（绝对不会空白名）
                save_path = osp.join(save_seq_dir, f'{seq_}.png')
                    # 保存图片
                cv2.imwrite(save_path, img)
                print(f"已保存：{save_path}")  # 可选：打印保存路径，方便排查
...
# 只从本地加载，绝对不联网、不下载
    if os.path.exists(model_path):
        print(f'✅ 本地模型存在，直接加载：{model_path}')
    else:
    # 如果本地没有，直接报错，不下载
        raise FileNotFoundError(f'❌ 模型文件不存在！请手动放入：{model_path}')
